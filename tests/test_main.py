...
def test_insert_points_by_num():
    xs = np.array([0, 3 / 4, 1])
    assert np.array_equal(insert_points_by_num(xs, 0), xs)
    assert np.array_equal(insert_points_by_num(xs, 1), np.array([0, 3 / 8, 3 / 4, 1]))
    assert np.array_equal(
        insert_points_by_num(xs, 2), np.array([0, 1 / 4, 1 / 2, 3 / 4, 1])
    )
    assert np.array_equal(
        insert_points_by_num(xs, 3), np.array([0, 1 / 4, 1 / 2, 3 / 4, 7 / 8, 1])
    )


def test_insert_points_by_dist():
    xs = np.array([0, 3 / 4, 1])
    assert np.array_equal(insert_points_by_dist(xs, 0), xs)
    assert np.array_equal(insert_points_by_dist(xs, 1 / 2), xs)
    assert np.array_equal(
        insert_points_by_dist(xs, 1 / 4), np.array([0, 1 / 4, 1 / 2, 3 / 4, 1])
    )
    assert np.array_equal(insert_points_by_dist(xs, 2), xs)
...
def test_sample_trip_points():
    ...
    # Point dist
    points_and_patterns = sample_trip_points(
        test_feed, [trip_id], method="distance", value=0.010
    )
    ...
    # Num points
    points_and_patterns = sample_trip_points(
        test_feed, [trip_id], method="num_points", value=30
    )
    assert len(points_and_patterns) == 1
    points, pattern = points_and_patterns[0]
    ...
    # Stop multiplier
    points_and_patterns = sample_trip_points(
        test_feed, [trip_id], method="stop_multiplier", value=1
    )
    assert len(points_and_patterns) == 1
    points, pattern = points_and_patterns[0]
